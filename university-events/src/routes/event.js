...
router.put('/events/:id([1-9][0-9]+|[1-9])', authorizeUser,
    body('title').isLength({min: 10, max: 128}),
    body('content').isLength({min: 20, max: 100000}),
    body('startDate').custom((value, {req}) => {
        const event = req.body;
    ...
    errorHandler(eventService.updateById));
router.delete('/events/:id([1-9][0-9]+|[1-9])', authorizeUser, errorHandler(eventService.deleteById));
